...
Printplots = function(){
  ...
  foreach(city = cityNames, .combine = "cfun") %do% {
    foreach(str = c("AP","HM","TP"), .combine = "cfun") %do% {
      ...
      colnames(df) = c("Time","val")
      ggplot(df) + geom_line(aes(x = Time, y = val, col = "blue")) +
        geom_line(aes(x = Time, y = rollmean(val, 24, na.pad = T), col = "red")) +
        BasicThemeWithLegend + labs(y = str) +
        scale_color_manual(values=c( "#BB933E","#255B87"),
                           label = c(str, "24-hour average")) + 
        theme(legend.title= element_blank())
      ggsave(Texcat("./basic_plots/",city,"_",str, ".png"), width = 8, height = 4.8, dpi = 200, units = "in")
    }
  }
  ...
  foreach(city = cityNames, .combine = "cfun") %do% {
      df <- get(city) %>% .[,c("WI","DR")]
      graph = plot.windrose(spd = df$WI, dir =  df$DR,
                    spdres = 2,
                    dirres = 22.5,
                    spdmin = 0,
                    spdmax = max(df$WI),
  ...
  PlotDiag <- function(data, mapping, ...) {
    ggplot(data, mapping = mapping) + geom_density(..., color = "#BB933E", alpha = 0.5, size = 1)
  }
  source("PointsWithLine.R")
  foreach(city = cityNames[-1], .combine = "cfun") %do% {
    foreach(str = pollutantNames, .combine = "cfun") %do% {
      
    df <- data.frame(get(cityNames[1]) %>% .[, str],get(city) %>% .[, str])
    colnames(df) = c(cityNames[1],city)
    graph <- ggpairs(df,
    ...
    ggsave(Texcat("./corr/",cityNames[1],"_",city,"_",str, ".png"), graph, width = 8, height = 4.5, dpi = 200, units = "in")
    }
  }
  ...
  # corr vs. beijing all
  df = data.frame(0,0,0)
  colnames(df) = c("City","Pollutant","Correlation")
  foreach(str = pollutantNames, .combine = "cfun") %do% {
    chunk <- get(str)[,-1]
    corr <- chunk[,-1] %>% apply(2,function(x) cor(x,chunk[,1]))
    chunk = data.frame(names(corr),rep(str,length(corr)),corr)
    colnames(chunk) = c("City","Pollutant","Correlation")
    df = rbind(df,chunk)
  }
      ...
      ggsave(Texcat("./corr/",cityNames[1],"_all.png"), graph, width = 8, height = 4.5, dpi = 200, units = "in")
  
  PlotCCF <- function() {
    foreach(str = pollutantNames, .combine = "cfun") %do% {
      env <- get(str)
      env$Month <- env$Time %>% as.POSIXct(format = "%m/%d/%Y %H:%M") %>% month() %>% as.factor()
      graph <- ggpairs(env[, -1],
                       columns = 1:12,
      ...
      graph
      ggsave(Texcat("Corr_", str, ".png"), graph, width = 42, height = 28, dpi = 200, units = "in")
    }
    ...
    foreach(str = pollutantNames, .combine = "cfun") %do% {
      ...
      df <- data.frame(-72:72)
      for (j in 1:12) {
        df[, j + 1] <- ccf(env[, 2], env[, 2 + j], plot = FALSE, lag.max = 72)$acf
      }
      colnames(df) <- c("Lag", cityNames[-2])
      df <- as.data.frame.array(melt(df, id = "Lag"))
      graph <- ggplot(df, aes(x = Lag, y = value)) +
        ...
        scale_shape_manual(values = 1:12) + BasicThemeNoLegend +
        labs(x = "Lag (x10 minutes)", y = Texcat("CCF (vs. ", str, " at Beijing)"))
    ...
    ggsave("ccf.png", width = 11, height = 9.6, dpi = 200, units = "in")
  }
  PlotCCF()
}
...
CreateLagData <- function(eData, index, lag, target) {
  ...
  foreach(str = polColumns, .combine = "cfun") %do% {
    polIntensity <- eData[, str] * (eData[, Texcat("CW.", gsub(".*\\.([0-9]{1,2})$", "\\1", str))] - minValue)
    ...
    newData[(1:ln) + suggestedLag] <- eData[, str]
    eData[, str] <- newData[1:ln] %>% na.interpolation(option = "linear")
  }
  colnames(df) <- c("Time", cityNames[-index])
  df <- df %>% melt("Time")
  ggplot() + geom_line(aes(x = Time, y = value, color = variable), df) +
    geom_point(aes(x = Time, y = value, color = variable, shape = variable), df[seq(144,ln*length(cityNames),288),] %>% na.omit(), stroke = 0.75) +
    BasicThemeWithLegend + theme(strip.text = element_text(size = rel(0.95), margin = margin(r = 6))) +
    scale_shape_manual(values = 1:12) +
    labs(y = Texcat("Suggested Lag (vs. Beijing's ", gsub("\\.[0-9]{1,2}$", "", target), ") (Hours)")) +
    guides(color = guide_legend(title = "City", nrow = 2), shape = guide_legend(title = "City", nrow = 2))
...
CompoundLinear <- function(lData, scaler, index, target, lag, current, nTest) {
  ...
  pred <- c(rep(NA, lag), (predict(model, x) * scaler[[1]][target] + scaler[[2]][target]) %>% exp())
  assign(Texcat("prdic.", GetParamText("c")), pred, envir = globalenv())
...
foreach(pol = pollutantNames[c(2,5)], .combine = "cfun") %do% {
  # foreach(city = cityNames, .combine = "cfun") %do% {
  city <- "Beijing"
  # pol <- "NO2"
  param <- c("AP.A", "HM.A", "TP.A", "CW.A",  Texcat(pollutantNames, ".A"))
  # SetQuery(city, pol, 1, T, param, 576, 64, 1, 100, 16, 30, 9)
  # DoRegressions(city, pol, lag, current , param, nTest, batch, step, epoch, numUnit, stop, split, s = T, c=T, l=T)
  # SetQuery(city, pol, 8, T, param, 576, 64, 8, 100, 16, 15, 9)
  # DoRegressions(city, pol, lag, current , param, nTest, batch, step, epoch, numUnit, stop, split, s = T, c=T, l=T)
  SetQuery(city, pol, 24, T, param, 576, 64, 24, 100, 16, 5, 9)
  DoRegressions(city, pol, lag, current , param, nTest, batch, step, epoch, numUnit, stop, split, s = T, c=T, l=T)
  # }
}

PrintPred = function(df,str){
  graph = ggplot(df) + geom_line(aes(x = Time, y = value, color = variable)) + BasicThemeWithLegend +
    theme(legend.title = element_blank(), legend.text = element_text(size = rel(0.95), margin = margin(r = 6)), legend.position = "bottom") +
    labs(y = pol)
  ggsave(Texcat("./prdic_image/", GetParamText(str), ".png"), graph, width = 8, height = 5, dpi = 200, units = "in")
}

PrintQQ = function(df,str){
  graph = ggplot(df) + geom_point(aes(x = actual, y = value, color = variable, alpha = 0.5)) +
    geom_abline(intercept = 0, slope = 1) + 
    theme(legend.title = element_blank(), legend.text = element_text(size = rel(0.95), margin = margin(r = 6)), legend.position = "bottom") +
    labs(x = Texcat("Actual ", pol), y = Texcat("Predicted ", pol))
  if (str == "a") {
    graph = graph + BasicThemeWithLegend
  } else {
    graph = graph + BasicThemeNoLegend
  }
  ggsave(Texcat("./qq_image/", GetParamText(str), ".png"), graph, width = 8, height = 5, dpi = 200, units = "in")
}
...
foreach(pol = pollutantNames, .combine = "cfun") %do% {
  ...
  # SetQuery(city, pol, l, T, param, 576, 64, l, ep[index], 16, sp[index], 9)
  SetQuery(city, pol, l, T, param, 576, 64, l, ep[index], 16, sp[index], 9)
  ...
  actual_lag <- get(city)[, pol] %>% tail(576 + lag) %>% head(576)
  ...
  # print(summary(cm))
  # 
  # df <- data.frame(Time, actual, Linear) %>% melt(id = "Time")
  # PrintPred(df,"s")
  # df <- data.frame(actual, Linear)%>% melt(id = "actual")
  # PrintQQ(df,"s")
  # 
  # df <- data.frame(Time, actual, Compound_Linear) %>% melt(id = "Time")
  # PrintPred(df,"c")
  # df <- data.frame(actual, Compound_Linear)%>% melt(id = "actual")
  # PrintQQ(df,"c")
  # 
  # df <- data.frame(Time, actual, LSTM_RNN) %>% melt(id = "Time")
  # PrintPred(df,"l")
  # df <- data.frame(actual, LSTM_RNN)%>% melt(id = "actual")
  # PrintQQ(df,"l")
  # 
  # df <- data.frame(Time, actual, Linear, Compound_Linear, LSTM_RNN) %>% melt(id = "Time")
  # PrintPred(df,"a")
  # df <- data.frame(actual, Linear, Compound_Linear, LSTM_RNN) %>% melt(id = "actual")
  # PrintQQ(df,"a")
  # }
  }
}
